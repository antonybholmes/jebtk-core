...
public class ColorUtils {
	...
	/**
	 * Return a grayscale color where 1 = 100% gray (i.e black) and 0 =
	 * 0% gray i.e. white.
	 *
	 * @param d the d
	...
	public static Color lighten(Color color, double d) {
		...
		v[2] = (float)Mathematics.bound(d, 0, 1); //(float)Mathematics.bound((1 - v[2]) * (1 + d), 0, 1); //(float)Math.min(1, v[1] * (1 + Math.max(0, Math.min(1, d))));
	...
	public static Color saturation(Color color, double d) {
		...
		// Adjust the saturation
		v[1] = (float)Mathematics.bound(v[1] + ((1 - v[1]) * (1 + d)), 0, 1); //(float)Math.min(1, v[1] * (1 + Math.max(0, Math.min(1, d))));
	...
	public static Color makeColor(float r, float g, float b, float a) {
		return new Color(Mathematics.bound(r, 0, 1),
				Mathematics.bound(g, 0, 1),
				Mathematics.bound(b, 0, 1),
