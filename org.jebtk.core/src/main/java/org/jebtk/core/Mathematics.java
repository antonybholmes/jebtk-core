...
/**
 * Extended mathematical functions not provided by the
 * standard Java Math library.
 * 
...
public class Mathematics {
	...
	/**
	 * Linear interpolation of function specified by points x y
	 * to evaluate y' of values.
	 *
	 * @param x the x
	 * @param y the y
	 * @param values the values
	...
	public static List<Double> linearInterpolation(List<Double> x,
		...
		for (int i = 0; i < values.size(); ++i) {
			...
			/*
			// Modified binary search to find points to
			// interpolate between. Assumes values are
			...
			while(maxs >= mins) {
				...
				if (indexed.get(mids).value <= v && indexed.get(mids + 1).value > v) {
					...
					xd = (v - indexed.get(mids).value) / (indexed.get(mids + 1).value - indexed.get(mids).value);
					...
					break;
				} else if (indexed.get(mids).value == indexed.get(mids + 1).value) {
					// In regions where there are multiple bins with the same
					// value, keep skipping forward one until we find a bin
					// boundary. This portion of the algorithm is searching
					// linearly, but for the most part, if the values are
					// reasonably unique, the algorithm will perform in N(log 2)
					++mins;
				} else if (indexed.get(mids).value > v) {
					maxs = mids - 1;
				} else if (indexed.get(mids).value < v) {
					mins = mids + 1;
				} else {
	...
	/**
	 * Bounds v and guarantees it lie between a number range.
	 * Chiefly for use for ensuring that p-values etc do not exceed 1 or
	...
	/**
	 * Generate a random sequence of integers between 0 and max - 1 up to a
	 * given size.
	 *
	 * @param <T> the generic type
	 * @param max the max
	...
	public static double[] tiedRank(double[] values) {
		...
		//ranks.put(sorted.get(0).getIndex(), rank / countMap.get(sorted.get(0).getValue()));
	...
	/**
	 * Compare two doubles and return -1 if v1 < v2, 1 if v1 > v2, or 0
	 * otherwise.
	 * 
	 * @param v1
	 * @param v2
