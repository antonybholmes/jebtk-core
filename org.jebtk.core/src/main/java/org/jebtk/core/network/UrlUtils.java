...
public class UrlUtils {
	...
	public static final Pattern GOOD_IRI_CHAR =
			Pattern.compile("a-zA-Z0-9\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF");
     ...
     */
    private static final Pattern IRI = 
    		Pattern.compile("[" + GOOD_IRI_CHAR + "]([" + GOOD_IRI_CHAR + "\\-]{0,61}[" + GOOD_IRI_CHAR + "]){0,1}");

    /** The Constant GOOD_GTLD_CHAR. */
    private static final Pattern GOOD_GTLD_CHAR =
    		Pattern.compile("a-zA-Z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF");
    
    /** The Constant GTLD. */
    private static final Pattern GTLD = 
    		Pattern.compile("[" + GOOD_GTLD_CHAR + "]{2,63}");
    
    /** The Constant HOST_NAME. */
    private static final Pattern HOST_NAME = 
    		Pattern.compile("(" + IRI + "\\.)+" + GTLD);
	...
	public static void mailto(List<String> recipients, 
		...
		String uriStr = String.format("mailto:%s?subject=%s&body=%s",
				TextUtils.join(recipients, ","), // use semicolon ";" for Outlook!
				urlEncode(subject),
				urlEncode(body));
	...
	/**
	 * Read the contents of an URL and cache it. Newlines
	 * are removed so this method is best for returning
	 * XML, JSON etc where it may be subsequently
	...
	public static String readUrl(URLConnection connection) throws IOException {
		...
		BufferedReader in = 
				FileUtils.newBufferedReader(connection.getInputStream());
	...
	/**
	 * Disables checking of sll certificates. This is intended for programs
	 * in a debug mode using self-signed certificates to prevent the JVM
	 * from complaining.
	 *
