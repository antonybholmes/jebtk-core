...
/**
 * The Class JsonBuilder builds json strings in a similar fashion to a 
 * general purpose StringBuilder. It assumes the user can correctly form
 * json with the constructs and relies upon users correctly calling
 * {@code startArray()} and {@code endArray()} and 
 * {@code startObject()} and {@code endObject()} to indicate where a new
 * JSON element should be inserted. This class is designed for speed and
 * does not offer pretty printing; JSON is written as a single line string.
 */
public class JsonBuilder implements CharSequence {
	...
	/**
	 * Keep track of how many items deep we are in an array or object so
	 * that we can decide whether to print a comma or not.
	 */
	private int mC = 0;
	...
	/**
	 * Create an object key consisting of an array of values. Values are
	 * converted to strings using {@code toString()}.
	 *
	 * @param name the name
