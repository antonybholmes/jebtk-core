...
/**
 * The Class Splitter takes a string and splits it into substrings according
 * to a delimiter or set of delimiters.
 */
public class Splitter {

	/**
	 * The default size of the list of splits. This is to reduce
	 * resizing operations on the list in most cases.
	 */
	...
	public static class CharSplitMode implements SplitMode {
		...
		/* (non-Javadoc)
		 * @see org.abh.common.text.Splitter.SplitMode#split(java.lang.String, org.abh.common.text.Trimmer, boolean, int)
		 */
		@Override
		public List<String> split(final String text, 
			...
			List<String> list = 
					new MaxSizeArrayList<String>(DEFAULT_SPLIT_SIZE, maxNumItems);
	...
	/**
	 * Split a string using any of the delimiters in the list to
	...
	/**
	 * Split a CSV string by commas (also taking into account values in
	 * quotation marks.
	 */
	...
	private boolean mIgoreEmptyStrings;
	 ...
	 */
	public Splitter(SplitMode splitMode, 
			Trimmer trimmer,
			boolean ignoreEmptyStrings,
			int maxNumItems) {
	...
	/**
	 * Splits some text into multiple sub strings, based on a delimiter
	 * or pattern.
	 *
	 * @param text the text
	 * @return the list
	...
	public Splitter trim(Trimmer trimmer) {
		return new Splitter(mSplitMode, 
				trimmer, 
				mIgoreEmptyStrings,
				-1);
	}
	...
	public static Splitter on(char delimiter) {
		if (delimiter == ',') {
			...
			return new Splitter(new CharSplitMode(delimiter), 
					Trimmer.NO_TRIM, 
					false,
					-1);
		}
	...
	public static Splitter on(char... delimiters) {
		return new Splitter(new CharsSplitMode(delimiters), 
				Trimmer.NO_TRIM, 
				false,
				-1);
	}
	...
	public static Splitter on(String delimiter) {
		return new Splitter(new TextSplitMode(delimiter), 
				Trimmer.NO_TRIM, 
				false,
				-1);
	}
	...
	public static Splitter on(Pattern regex) {
		return new Splitter(new PatternSplitMode(regex), 
				Trimmer.NO_TRIM, 
				false,
				-1);
	}
	...
	public static Splitter onComma() {
		return new Splitter(new CSVSplitMode(), 
				Trimmer.NO_TRIM, 
				false,
				-1);
	}
	 ...
	 * and splits each string and adds the key value pairs to a map.
	 * Suitable only if keys are unique.
	 *
