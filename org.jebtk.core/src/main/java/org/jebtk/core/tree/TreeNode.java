 ...
 */
public class TreeNode<T> implements IdProperty, Iterable<TreeNode<T>>, Reversible<TreeNode<T>>, Comparable<TreeNode<T>>, Cloneable, TreeNodeEventProducer, XmlRepresentation, JsonRepresentation, Serializable, TreeNodeEventListener  {
	 ...
	 */
	private TreeNodeEventListeners mListeners = 
			new TreeNodeEventListeners();

	/**
	 * The member children.
	 */
	protected List<TreeNode<T>> mChildren = 
			new ArrayList<TreeNode<T>>();
	...
	/**
	 * Create node with a name, but no data. Typically
	 * used for creating folder nodes.
	 *
	 * @param name the name
	 */
	...
	/**
	 * Creates a clone of the node including all of its children.
	 * If the value is not immutable, this method should be
	 * overridden to ensure that value is properly cloned as well.
	 *
	 * @param node the node
	 */
	...
	/**
	 * Returns the total number of children under this parent (i.e all
	 * children and children of children etc).
	 *
	...
	/**
	 * Can be used to forcibly indicate that a node is
	 * a parent, for example a folder node that is
	 * currently empty.
	 *
	...
	/**
	 * Sets whether the node should be displayed
	 * expandable or not.
	 *
	...
	/**
	 * Sets whether the node should be displayed
	 * collapsed or not and controls whether a node changed
	 * event should be fire. This is used by batch
	 * methods to do bulk expand operations on multiple nodes
	 * without fireing node events for each change.
	 *
	...
	public static <X> void updateExpanded(TreeNode<X> root, 
		...
		while (!stack.isEmpty()) {
			...
			if (node.mIsExpandable && node.mChildren.size() > 0) {
	...
	/**
	 * Allow a node to inherit the children of another node. This will result
	 * in nodes sharing child nodes. Use close to create copies of children
	 * so that the node can be updated independently.
	 *
	 * @param node the node
	 */
	...
	/**
	 * Search a tree using a path of ids. Since all
	 * nodes have a unique id, this method guarantees
	 * the correct node will be returned along a 
	 * valid path.
	 *
	 * @param path the path
	 * @return the child
	...
	/**
	 * Search a tree using a path of ids. Since node names are not
	 * guaranteed to be unique, this method may not find the correct branch
	 * if you allow multiple nodes with the same name.
	 * 
	 *
	 * @param path the path
	 * @return the child
	...
	/**
	 * Search the tree for a node containing the search text.
	...
	/**
	 * Search the tree for a node whose name equals the search text.
	 * Returns null if node not found.
	 *
	 * @param search the search
	 * @return the tree node
	...
	/**
	 * Return the list of nodes corresponding to
	 * the path.
	 *
	 * @param path the path
	 * @return the chain
	...
	public TreeNodeChain<T> getChain(TreePath path) {
		...
		TreeNodeChain<T> chain = new TreeNodeChain<T>();
	...
	/**
	 * Translates a tree path into a path. This can make
	 * it easier to see a path by name rather than ids to
	 * make it more human readable. Since trees may contain
	 * multiple nodes with the same name, paths using
	 * node names are not guaranteed to be unique, therefore
	 * it is advisable to use TreePath to search a tree.
	 * 
	...
	/**
	 * Returns a path so that a path can be followed from
	 * this node's root to the node.
	 *
	 * @return the path
	 */
	...
	/**
	 * Return the node value as a string. Returns Integer.MIN_VALUE if there
	 * is an error.
	 *
	 * @return the int
	 */
	...
	/* (non-Javadoc)
	 * @see org.abh.common.tree.TreeNodeEventListener#nodeChanged(org.abh.common.event.ChangeEvent)
	 */
	@Override
	public void nodeChanged(ChangeEvent e) {
		countCumulativeChildren();
	...
	/* (non-Javadoc)
	 * @see org.abh.common.tree.TreeNodeEventListener#nodeUpdated(org.abh.common.event.ChangeEvent)
	 */
	@Override
	public void nodeUpdated(ChangeEvent e) {
		// Forward events
	...
	public Iterable<TreeNode<T>> reversed() {
		return new ReverseIterator<TreeNode<T>>(mChildren);
	}
}
